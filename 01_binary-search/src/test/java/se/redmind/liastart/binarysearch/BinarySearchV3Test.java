...
public class BinarySearchV3Test {
    ...
    private BinarySearchV3 search = new BinarySearchV3() {
        public int chop3(int key, int[] values, int start, int end) {
            int found = -1;
            int mid;
            ...
            if(start < end){
                ...
                if (key < values[mid - 1]) {
                    if (mid == 1) {
                        ...
                        found = chop3(key, values, start, end);
                    }
                }
                if (key == values[mid - 1]) return mid-1;
                if (key > values[mid - 1]) {
                        start = mid;
                        found = chop3(key, values, start, end);
                }
...
//
    ...
    private int chop(int key, int[] values) {
        ...
        return search.chop3(key, values, 0, values.length-1);
